...
*/
    ...
    group_1->addToGroup(scene->addLine(width/2,0,width/2,height,penBlack));
    group_1->addToGroup(scene->addLine(0,height/2,width,height/2,penBlack));

    for(int i = 0; i < ToDraw1.size(); i++) //i-ый график
    {
        for(int j = 0; j < ToDraw1[i].size()-1; j++) //j-ый точка
        {
            group_2->addToGroup(scene->addLine(ToDraw1[i][j].first,ToDraw1[i][j].second,
                                               ToDraw1[i][j+1].first,ToDraw1[i][j+1].second, penRed));
        }
    }
 ...
 * */
void graphics::deleteItemsFromGroup(QGraphicsItemGroup *group)
{
...
void graphics::FillToDraw1Vec(vector<float> &vecf)
{
    vector<pair<float,float>> v;
    for(int i = 0; i < vecf.size(); i++)
    {
        v.push_back(make_pair(i+1, vecf[i]*1000));
    }
    ToDraw1.push_back(v);
}
